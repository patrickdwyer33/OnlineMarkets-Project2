...
class EW:
    ...
    def payoff_cacluator(self, bids, a):
        other_bids = bids.copy()
        other_bids[self.bidder] = self.q * a
        winner = (0, other_bids[0])
        for i in range(1, len(other_bids)):
            if other_bids[i] > winner[1]:
                winner = (i, other_bids[i])
        if bidder == winner[0]:
            return (self.value - (winner[1]/self.q))
        else:
            return 0
